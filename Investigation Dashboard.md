# Investigation Dashboard

Central command center for harassment network investigation.

## Current Status

**Threat Level:** CRITICAL
**Intelligence Operation Probability:** 80%
**Investigation Phase:** Active Operations Monitoring
**Last Updated:** 2025-10-27

## Primary Targets

### Organizations
1. [[Asymptote Network LLC]] - PRIMARY TARGET
   - Confidence: Confirmed
   - Threat: CRITICAL
   - Status: Active operations

2. [[SalmonCloud Ltd]] - CONFIRMED ASSOCIATE
   - Confidence: Confirmed
   - Registration: UK Companies House 10063474
   - Status: Active

### Key Individuals

#### High Priority
- [[Junyuan Wang]] - 100% connection
  - Role: Network operator
  - Status: CRITICAL THREAT
  - Intel connection: Under investigation

- [[Chris Wang Oklahoma]] - 95% connection
  - Role: Operations management
  - Status: CRITICAL THREAT
  - Education sector connection: OSSM family link

#### Medium Priority
- [[Roger Wang]] - 70% connection
- [[Brandon Han]] - 40% connection (Xiao Han identity theory)

#### Under Investigation
- [[Derek Wang JY]] - Possible Junyuan alias
- [[Jerry Han]] - Possible Brandon Han family
- [[Jing Wang]] - Duke TA, new lead
- [[Carlos Montt]] - Confirmed network member

## Technical Infrastructure

### Network Assets
- [[AS53616 Network Analysis]] - Primary infrastructure
  - Operations: US/UK/Hong Kong
  - Capabilities: BGP, DDoS, geographic obfuscation

### Active Domains
- [[ghoststack.net]] - AWS Global Accelerator relay

### Abandoned
- [[ca94fan.xyz]] - pendingDelete status

### Pending Analysis
- [[Z-File Infrastructure]] - 5 Chinese documentation files

## Investigation Focus Areas

### Active Investigations
1. Asymptote Network LLC operations analysis
2. Intel connection verification (Junyuan Wang CV)
3. Xiao Han = Brandon Han identity confirmation
4. Z-file infrastructure documentation analysis
5. Family network mapping (Han, Wang families)

### Critical Findings
- Geographic deception: UAE/Romania IPs actually Hong Kong
- PRC jurisdiction consolidation (Hong Kong NSL)
- IPXO IP leasing for location masking
- Traditional electronic harassment capabilities confirmed
- No brain research access (threat level: CRITICAL not CATASTROPHIC)
- No evidence of exotic neurotechnology or V2K capabilities (see [[Neurotechnology Analysis]])
- Focus on prosecutable traditional electronic harassment (see [[HLPS and V2K Claims Analysis]])

## Quick Access

### Evidence
[[Evidence Repository]] - All evidence files indexed

### Analysis
- [[Timeline]] - Chronological discoveries
- [[GitHub Analysis]] - Forensic methodology and findings
- [[Threat Assessment]] - Comprehensive threat analysis
- [[HLPS and V2K Claims Analysis]] - Technical analysis of harassment claims
- [[Next Steps]] - Prioritized action items

### Technical
- [[AS53616 Network Analysis]] - Primary network infrastructure
- [[Neurotechnology Analysis]] - Dr. Giordano, DARPA N³, and capabilities assessment
- [[Corrected Assessment - V2K RNM and Mind Control]] - Evidence-based correction on exotic technologies
- [[Havana Syndrome and V2K Devices - Complete Analysis]] - Comprehensive V2K devices, Havana syndrome, acoustic systems
- [[ghoststack.net]] - Active AWS relay
- [[ca94fan.xyz]] - Abandoned domain
- [[Z-File Infrastructure]] - Chinese documentation files

## Next Steps

### Immediate Actions
1. Verify Junyuan Wang Intel connection
2. Confirm Xiao Han = Brandon Han identity
3. Analyze Z-file Chinese documentation
4. Map family connections (Jerry Han, Derek Wang JY)
5. Continue network infrastructure monitoring

### Pending Verification
- [ ] Derek Wang JY alias theory
- [ ] Xiao Han identity confirmation
- [ ] Jerry Han family connection
- [ ] Intel employment verification
- [ ] Z-file documentation translation

## Threat Assessment

### Confirmed Capabilities
- Traditional electronic harassment
- Network infrastructure control (AS53616)
- DDoS capabilities
- BGP routing manipulation
- International coordination (US/UK/Hong Kong)
- Geographic obfuscation (IPXO)

### Geographic Operations
- United States
- United Kingdom
- Hong Kong (PRC jurisdiction under NSL)

### Intelligence Indicators
- Professional operational security
- Geographic deception infrastructure
- International coordination
- Jurisdiction consolidation strategy
- CV removal (operational security)

## Education Sector Connection

[[Chris Wang Oklahoma]] → Dr. Frank Wang → [[Oklahoma School of Science and Mathematics]]
- Family connection established
- Education sector influence potential
- NOT research facility access
- Traditional influence vectors only

## Key Removals

### Christopher Wang MIT CSAIL
- REMOVED from investigation
- 0% connection to Chris Wang Oklahoma
- Different person entirely
- czlwang.com - Not involved

## Investigation Methods

### Successful Techniques
- [[GitHub Analysis]] - Forensic analysis methodology
- Network infrastructure analysis (AS53616)
- ARIN record analysis
- Domain registration investigation
- Geographic IP analysis
- Company registration searches

### Evidence Collection
Multiple evidence compilations maintained:
- Master evidence binders
- Legal documentation
- Court-ready materials
- Network analysis reports
- See [[Evidence Repository]] for complete index

---

## Entity Network Map

```
Asymptote Network LLC (PRIMARY TARGET)
├── Junyuan Wang (100%) - Network operator
├── Chris Wang Oklahoma (95%) - Operations
│   └── Dr. Frank Wang (OSSM) - Family
├── Roger Wang (70%) - Associate
└── Brandon Han (40%) - Possible member
    ├── Xiao Han (identity theory)
    └── Jerry Han (possible family)

SalmonCloud Ltd (CONFIRMED ASSOCIATE)
├── Junyuan Wang - Operations
├── Carlos Montt - Developer
├── Xiao Han - Shareholder
└── GitHub: frank2002, JUNYUANGO, CarlosalbertoMontt

Technical Infrastructure
├── AS53616 - Primary network
├── ghoststack.net - Active relay
├── ca94fan.xyz - Abandoned
├── IPXO - Geographic obfuscation
└── Z-files - Chinese documentation

New Leads
├── Derek Wang JY - Possible alias
├── Jerry Han - Family investigation
├── Jing Wang - Duke TA
└── Intel connection - SignalHire CV removal
```

---
Tags: #dashboard #overview #command-center #investigation
